...
Params::Params(std::string filename) : m_params_filename(filename){
    ...
    m_omega_radiation = 2.471e-5*pow(m_T_cmb/2.725f,4.0f)/pow(m_hubble,2.0f);
...
void Params::dump(){
    ...
    for (int i = 0; i < m_nsteps; i++){
        if (m_pk_dumps[i]) std::cout << i << " ";
    }
    std::cout << "\n";

    std::cout << "PARTICLE_DUMP ";
    for (int i = 0; i < m_nsteps; i++){
        if (m_particle_dumps[i]) std::cout << i << " ";
    }
    std::cout << "\n";
...
double Params::z_ini() const{
    return m_z_ini;
}

double Params::z_fin() const{
    return m_z_fin;
}

int Params::nsteps() const{
    return m_nsteps;
}
...
double Params::rl() const{
    return m_rl;
}

int Params::seed() const{
    return m_seed;
}

double Params::w_de() const{
    return m_w_de;
}

double Params::wa_de() const{
    return m_wa_de;
}

double Params::omega_cb() const{
    return m_omega_cb;
}

double Params::f_nu_massless() const{
    return m_f_nu_massless;
}

double Params::f_nu_massive() const{
    return m_f_nu_massive;
}

double Params::omega_radiation() const{
    return m_omega_radiation;
}

double Params::omega_matter() const{
