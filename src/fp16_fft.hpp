...
class HalfFFT : public FFT<complexHalfDevice> {
  private:
    ...
    const std::string fmt_type_name =
        std::string(typeid(complexHalfDevice).name());
    ...
    void fft(complexHalfDevice* in, complexHalfDevice* out, int direction) {
        ...
        gpufftResult result;
        if ((result = gpufftXtExec(m_plan, in, out, direction)) !=
            GPUFFT_SUCCESS) {
            LOG_ERROR("Error in gpufftXtExec: %s",
                      gpufftResult_to_string(result));
            exit(1);
        }
  ...
  public:
    HalfFFT(int ng) : m_ng(ng), m_n{m_ng, m_ng, m_ng} {
        ...
        if ((result = gpufftCreate(&m_plan)) != GPUFFT_SUCCESS) {
            LOG_ERROR("Error in gpufftCreate: %s",
                      gpufftResult_to_string(result));
            exit(1);
        }
        if ((result = gpufftXtMakePlanMany(
                 m_plan, m_rank, m_n, NULL, 1, 1, CUDA_C_16F, NULL, 1, 1,
                 CUDA_C_16F, 1, &m_work_size, CUDA_C_16F)) != GPUFFT_SUCCESS) {
            LOG_ERROR("Error in gpufftXtMakePlanMany: %s",
                      gpufftResult_to_string(result));
            exit(1);
        }
    }
...
template <class T> class CastHalfFFT : public FFT<T> {
  private:
    int m_ng;
    HalfFFT m_fft;
    int m_blockSize;
    ...
    void fft(T* in, T* out, int direction) {
        ...
        gpu_allocator.alloc(&d_array,
                            sizeof(complexHalfDevice) * m_ng * m_ng * m_ng);
        launch_copy_grid(in, d_array, m_ng * m_ng * m_ng, m_numBlocks,
                         m_blockSize);
        if (direction == GPUFFT_FORWARD)
            m_fft.forward(d_array);
        else
            m_fft.backward(d_array);
        launch_copy_grid(d_array, out, m_ng * m_ng * m_ng, m_numBlocks,
                         m_blockSize);
        gpu_allocator.free(d_array);
    }

  public:
    CastHalfFFT(int ng, int blockSize = 32)
        : m_ng(ng), m_fft(m_ng), m_blockSize(blockSize),
          m_numBlocks((m_ng * m_ng * m_ng + (m_blockSize - 1)) / m_blockSize) {}
    ~CastHalfFFT() {}
