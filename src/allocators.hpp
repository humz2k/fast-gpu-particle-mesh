...
static inline void show_alive_allocations(const std::unordered_map<void*, size_t>& allocations, const char* name){
    LOG_DEBUG(" ");
    LOG_DEBUG("##############################");
...
class GPUAllocator{
    ...
    ~GPUAllocator(){
        ...
        LOG_INFO("GPU Allocations (total = %lu bytes, current = %lu bytes)",total_size,current_size);
    }

    template<class T>
    inline int alloc(T** ptr, size_t sz){
    ...
    inline int free(T* ptr){
        ...
        LOG_DEBUG("freeing GPU pointer %p (previous size %lu bytes)",(void*)ptr,sz);
...
class CPUAllocator{
    ...
    ~CPUAllocator(){
        ...
        LOG_INFO("CPU Allocations (total = %lu bytes, current = %lu bytes)",total_size,current_size);
    }
    ...
    inline int free(T* ptr){
        ...
        LOG_DEBUG("freeing CPU pointer %p (previous size %lu bytes)",(void*)ptr,sz);
        current_size -= sz;
